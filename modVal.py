...
def appendcol(data,col,pos):
	for i in range(0,len(data)):
		data[i].insert(pos,col[i])
	return data
...
# # replace everything back to string
# for i in range(0,len(listOfAllTrain)):
# 	listOfAllTrain[i][11]=str(fincol12[i])
# removeCol(listOfAllTrain,0)


f=[]
for i in range(0,len(listOfAllTrain[0])):
	f.append('mvar' + str(i))

listOfAllTrain.insert(0,f)

with open("modLeader.csv", "wb") as f:
    writer = csv.writer(f)
    writer.writerows(listOfAllTrain)